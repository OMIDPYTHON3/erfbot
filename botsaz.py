...
async def Start(client, message):
    if message.from_user.id in ONWER:
        ...
        await message.reply("خوش آمدید، انتخاب کنید", reply_markup=reply)
    ...
    if message.from_user.id in ONWER:
        ...
        if token.text == "بازگشت":
            ...
            for i in token.text.splitlines():
                await message.reply("ربات فعال شد", reply_markup=ReplyKeyboardMarkup(keyboard=[["ساخت ربات"]], resize_keyboard=True))
                attacker = Bot()
